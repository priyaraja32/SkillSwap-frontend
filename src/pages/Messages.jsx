...
export default function Messages() {
  ...
  //  Register socket with userId on mount
  useEffect(() => {
    if (currentUserId) {
      socket.emit("register", currentUserId);
      console.log(" Registered socket with userId:", currentUserId);
    }
  }, [currentUserId]);

  //  Fetch conversations + all users
  useEffect(() => {
    Promise.all([
      api.get("/users"),
      api.get("/messages"),
  ...
  //  Receive messages via socket (real time)
  useEffect(() => {
    socket.on("receive_message", (msg) => {
      ...
      if (
        ...
        setMessages((prev) => {
          ...
          const alreadyExists = prev.find((m) => String(m._id) === String(msg._id));
  ...
  //  Merge conversations + other users for left panel
